...
{
    /// <summary>
    /// Implements the forward checking backtracking algorithm to solve sudokus.
    /// </summary>
    static class ForwardCheckingBacktrackSolver
    {
        #region Solve
        /// <summary>
        /// Solves a sudoku with our standard implementation of Forward Checking.
        /// </summary>
        ...
        public static void Solve(Sudoku sudo, out long expanded)
        {
            ...
            while (i >= 0 && i < varCount)  // If i is out of range of the variables we know we that we a solution of failure
            {
                ...
                else
                {
                    ...
                    if (valueFound) i++;        // We broke out the while loop because we found a value, up i to take the next variable next iteration
                    else                        // Every value from the domain tried, we need to do backtracking
                    {
                        varTrials[i] = 0;       // Reset the varTrail
        ...
        /// <summary>
        /// Solves a sudoku with our extended implementation of Forward Checking.
        /// </summary>
        /// <param name="sudo">The sudoku to solve</param>
        /// <param name="expanded">Keeps track of the amount of iterations/node expansions of the algorithm</param>
        public static void SolveLL(Sudoku sudo, out long expanded)
        {
            ...
            while (i >= 0 && i < varCount)  // If i is out of range of the variables we know we that we a solution of failure
            {
                ...
                if (backtrack)              // First handle backtracking because of the hashsets
                {
                    sudo.ResetAndUpdate(v); // Reset the value of the tile
                    backtrack = false;      // Disable backtrack
                }
                else
                {
                    ...
                    bool valueFound = false;                            // Enables us to check whether we found a value that causes no empty domains
                    int varTrial = varTrials[i];                        // Get the varTrial of the tile
